"""
copied from 20 and change T conv -> linear
no batchnorm, using LeakyRelu in generator instead of Relu.
"""
...
class TransportT(torch.nn.Module):
    def __init__(self, input_shape = [3,32,32], z_dim = 100):
        ...
        self.main = nn.Sequential(
            ...
            *discriminator_block(conv_channel*2, conv_channel*4, bn=False),
        )
    ...
    def forward_T(self, x):
        x = x.view(x.shape[0], *self.input_shape)
        x = self.main(x).view(x.shape[0], self.output_main)
        x = self.fc_T(x)
        return x
...
class TransportG(torch.nn.Module):
    def __init__(self, z_dim = 100, output_shape = [1,32,32]):
        ...
        self.conv_blocks = nn.Sequential(
            ...
            # size: 1024 x 16 x 16
            nn.Conv2d(self.init_channel * 4, self.init_channel * 2, 3, stride=1, padding=1),
            ...
            nn.LeakyReLU(0.2,inplace=True),
            # size: 256 x 32 x 32
            nn.Conv2d(self.init_channel * 1, output_shape[0], 3, stride=1, padding=1),
            nn.Tanh(),
            # size: 3 x 32 x 32
        )
