"""
    Copied from transports23.
    Different G. fc_phi is single linear
    same as transports 26 in Yifei
"""
...
class TransportT(torch.nn.Module):
    def __init__(self, input_shape = [3,32,32], z_dim = 100):
        ...
        self.fc_T = nn.Sequential(nn.Linear(self.output_main, z_dim))
...
class TransportG(torch.nn.Module):
    def __init__(self, z_dim = 100, output_shape = [1,32,32]):
        ...
        self.init_size = 32 // 4
        self.init_shape = (self.init_channel * 2, self.init_size, self.init_size)
        ...
        self.l1 = nn.Sequential(
            ...
            nn.BatchNorm1d(1024),
            nn.LeakyReLU(0.2, inplace=True),
            nn.Linear(1024, np.prod(self.init_shape)),
            )
        # size: 1024 x 8 x 8
        ...
        self.conv_blocks = nn.Sequential(
            ...
            # size: 1024 x 16 x 16
            nn.Conv2d(self.init_channel * 2, self.init_channel, 5, stride=1, padding=2),
            nn.BatchNorm2d(self.init_channel, 0.8),
            nn.LeakyReLU(0.2,inplace=True),
            nn.Upsample(scale_factor=2),
            # size: 256 x 32 x 32
            nn.Conv2d(self.init_channel, output_shape[0], 5, stride=1, padding=2),
            nn.Tanh(),
            # size: 3 x 32 x 32
        )
